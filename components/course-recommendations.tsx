...
export default function CourseRecommendations({ currentCourseId, limit = 6 }: CourseRecommendationsProps) {
  ...
  const recommendations = useMemo(() => {
    ...
    const sameCategory = coursesData.filter(
      (c) => c.category === currentCourse.category && c.id !== currentCourseId && !savedCourseIds.has(c.id),
    )
  ...
  return (
    <section className="py-12">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-sm mt-1">Based on your learning interests</p>
      </div>
